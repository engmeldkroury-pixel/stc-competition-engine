...
def decide_bridge_event(event_id: str, payload: dict) -> dict:
    ...
    # Post-incident control: Capital no longer uses the deliberately relaxed
    # competition-opportunity gate. The 2026-09-24 loss review showed that
    # setups with weak 2h/4h alignment or an opposing monthly regime could pass
    # that gate. All competition lanes now require the strict A+ confirmation
    # gate before a locked plan may be created.
    ...
    gate_name = "high_conviction_gate"
    ...
    result_dict["setup_grade"] = (
        "A_PLUS" if final_gate_passed else "MONITOR_ONLY"
    )
