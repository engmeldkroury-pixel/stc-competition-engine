...
function positionHtml(p){
 ...
 return '<div class="card">'
  ...
  +'<div class="row"><span>Unrealized P/L estimate</span><span class="value '+(m.unrealized_pnl_usd==null?'wait':Number(m.unrealized_pnl_usd)>=0?'ok':'bad')+'">'+(m.unrealized_pnl_usd==null?'Unavailable':'  +(m.suggested_stop?'<div class="row"><span>Suggested stop</span><span class="value">'+num(m.suggested_stop)+'</span></div>':'')
...
function closedPositionHtml(p){
 return '<div class="card">'
  ...
  +'<div class="row"><span>Realized P/L</span><span class="value '+(Number(p.realized_pnl_usd||0)>=0?'ok':'bad')+'">
...
+num(openPnl,2)+(unknown>0?' • PARTIAL; '+unknown+' open trade(s) waiting for an STC mark':'')
 ...
 return '<div class="orderbox"><div class="small">TRADE INVENTORY — THIS COMPETITION</div>'
  ...
  +'<div class="row"><span>Realized P/L</span><span class="value '+(realized>=0?'ok':'bad')+'"> if(!account)return '<div class="small">Account state unavailable.</div>';
 const s=riskSummaryFor(competitionId);
 const tradeRisk=Number(account.equity_usd)*Number(account.risk_fraction);
 const portfolioCap=tradeRisk*6;
 const clusterCap=tradeRisk*3;
...
+num(realized,2)+'</span></div>'
  +'<div class="row"><span>Tracked P/L</span><span class="value '+(trackedPnl==null?'wait':trackedPnl>=0?'ok':'bad')+'">'+(trackedPnl==null?'Incomplete — one or more open marks unavailable':' if(!account)return '<div class="small">Account state unavailable.</div>';
 const s=riskSummaryFor(competitionId);
 const tradeRisk=Number(account.equity_usd)*Number(account.risk_fraction);
 const portfolioCap=tradeRisk*6;
 const clusterCap=tradeRisk*3;
